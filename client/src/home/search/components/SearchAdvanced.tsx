...
const SearchAdvanced: FC<Props> = ({ setMatchaNotif }) => {
  const [bodyRequest, setBodyRequest] = useState<SearchAdvanceRequestType | null>(null);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (tallMin && (tallMin < 50 || tallMin > 250)) {
      setMatchaNotif("La taille minimale doit être compris entre 50cm et 250cm");
      return;
    }
    ...
    if (tallMin && tallMax && tallMin > tallMax) {
      setMatchaNotif("La taille minimale doit être inférieure à la taille maximale");
      return;
    }
    setBodyRequest({
