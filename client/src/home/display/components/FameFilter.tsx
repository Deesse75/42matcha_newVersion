...
const FameFilter: FC<Props> = ({ listingName, setListing, setMatchaNotif }) => {
  ...
  const handleClick = () => {
    setReqData({ fameMin: refFame.current?.value ? parseInt(refFame.current.value) : 0 });
  };

  useEffect(() => {
    if (listingName === 'matcha')
      setUrlFilter(listingRoute.getFameFilterMatchaList);
    else if (listingName === 'view')
      setUrlFilter(listingRoute.getFameFilterViewList);
    else if (listingName === 'like')
      setUrlFilter(listingRoute.getFameFilterLikeList);
  }, []);
  ...
  return (
    <>
      <div className='dashboard_filter_fame'>
        <div className='dashboard_filter_fame_text'>Popularité supérieur à</div>
          <input
          ref={refFame}
            type='number'
            min={0}
