...
const DisplayMiniUserData: FC<Props> = ({profile}) => {
  ...
  useEffect(() => {
    ...
    else if (profile.county) setLocation(`${profile.county}, ${profile.region}`);
    else setLocation(profile.region);
    if (profile.gender && profile.orientation) setPreferences(`${profile.gender}, ${profile.orientation}`);
    else if (profile.gender && !profile.orientation) setPreferences(profile.gender);
    else if (!profile.gender && profile.orientation) setPreferences(profile.orientation);
    else setPreferences('');
  }, [profile]);
