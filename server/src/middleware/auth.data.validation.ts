...
export const authUserValidation = async (
  ...
  try {
    if (req.path === '/signup') {
      ...
      if (await mysql.getOneUserData(query, [req.body.email])) {
        res.status(400).json({ message: 'Adresse email déjà utilisée.' });
        return;
      } else {
        query = 'SELECT * FROM User WHERE username = ?';
        if (await mysql.getOneUserData(query, [req.body.username])) {
          res
            .status(400)
      ...
      if (req.path === '/signin_username') {
        query = 'SELECT * FROM User WHERE username = ?';
        existingUser = await mysql.getOneUserData(query, [req.body.username]);
        if (!existingUser) {
        ...
        query = 'SELECT * FROM User WHERE email = ?';
        existingUser = await mysql.getOneUserData(query, [req.body.email]);
        if (!existingUser) {
        ...
        query = 'SELECT * FROM User WHERE email = ?';
        existingUser = await mysql.getOneUserData(query, [
          req.body.email,
        ]);
        if (!existingUser) {
